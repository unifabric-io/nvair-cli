...
const (
	DefaultBaseURL = constant.DefaultAPIEndpoint
...
func (c *Client) GetSSHKeys() ([]GetSSHKeyResponse, error) {
	...
	endpoint := fmt.Sprintf("%s/?%s", sshKeysEndpoint, query.Encode())
...
func (c *Client) CreateService(simulationID, interfaceID, serviceName string, destPort int, serviceType string) (*EnableSSHResponse, error) {
	...
	endpoint := interfaceServicesEndpoint
...
// StartSimulation starts a stored simulation via the v3 lifecycle endpoint.
func (c *Client) StartSimulation(simulationID string) (*SimulationInfo, error) {
	endpoint := fmt.Sprintf("%s/%s/start/", simulationsEndpoint, simulationID)
	logging.Verbose("StartSimulation: Sending PATCH request to %s", endpoint)

	resp, err := c.doRequest("PATCH", endpoint, map[string]interface{}{}, true)
	if err != nil {
		logging.Verbose("StartSimulation: Request failed with error: %v", err)
		return nil, err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		logging.Verbose("StartSimulation: Received status code %d with body: %s", resp.StatusCode, string(bodyBytes))
		return nil, fmt.Errorf("start simulation failed: status %d: %s", resp.StatusCode, string(bodyBytes))
	}

	var simResp SimulationInfo
	if err := json.Unmarshal(bodyBytes, &simResp); err != nil {
		logging.Verbose("StartSimulation: Failed to decode response: %v", err)
		return nil, fmt.Errorf("failed to decode start simulation response: %w", err)
	}

	logging.Verbose("StartSimulation: Successfully requested start for simulation %s, current state: %s", simulationID, simResp.State)
	return &simResp, nil
}
...
// GetSimulationByID retrieves details for a specific simulation.
func (c *Client) GetSimulationByID(simulationID string) (*SimulationInfo, error) {
	endpoint := fmt.Sprintf("%s/%s/", simulationsEndpoint, simulationID)
	logging.Verbose("GetSimulationByID: Sending GET request to %s", endpoint)

	resp, err := c.doRequest("GET", endpoint, nil, true)
	if err != nil {
		logging.Verbose("GetSimulationByID: Request failed with error: %v", err)
		return nil, err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		logging.Verbose("GetSimulationByID: Received status code %d with body: %s", resp.StatusCode, string(bodyBytes))
		return nil, fmt.Errorf("get simulation failed: status %d: %s", resp.StatusCode, string(bodyBytes))
	}

	var simResp SimulationInfo
	if err := json.Unmarshal(bodyBytes, &simResp); err != nil {
		logging.Verbose("GetSimulationByID: Failed to decode response: %v", err)
		return nil, fmt.Errorf("failed to decode simulation response: %w", err)
	}

	logging.Verbose("GetSimulationByID: Successfully retrieved simulation %s in state %s", simResp.ID, simResp.State)
	return &simResp, nil
}
...
func (c *Client) DeleteServiceByID(serviceID string) error {
	...
	endpoint := fmt.Sprintf("%s%s/", interfaceServicesEndpoint, serviceID)
...
func (c *Client) GetNodes(simulationID string) ([]Node, error) {
	...
	for _, n := range rawResp.Results {
		listResp.Results = append(listResp.Results, normalizeNode(n))
	}
...
func normalizeNode(n rawNode) Node {
	metadata := ""
	switch value := n.Metadata.(type) {
	case string:
		metadata = value
	case nil:
		metadata = ""

...
func (c *Client) DeleteSimulation(name string) error {
	...
	logging.Verbose("DeleteSimulation: Found simulation ID: %s, sending DELETE request to %s", simulationID, endpoint)

	resp, err := c.doRequest("DELETE", endpoint, nil, true)
	if err != nil {
		logging.Verbose("DeleteSimulation: Request failed with error: %v", err)
		return err
	}
	defer resp.Body.Close()
	...
	// Check for errors
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		logging.Verbose("DeleteSimulation: Received status code %d with body: %s", resp.StatusCode, string(bodyBytes))

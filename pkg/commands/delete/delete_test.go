...
func TestExecute_MissingMappedFields(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "usage: nvair delete simulation <name> | nvair delete forward <forward-name>") {
...
func TestExecute_DeleteForwardByName(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/v3/simulations/nodes/interfaces/services/svc-2/" && r.Method == "DELETE":
			deleteCalled = true
	...
	if !deleteCalled {
		t.Fatalf("expected DELETE /v3/simulations/nodes/interfaces/services/svc-2/ to be called")
	}
...
func TestExecute_DeleteForwardByName_NonManagedPortSkipsIPTablesCleanup(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/v3/simulations/nodes/interfaces/services/svc-1/" && r.Method == "DELETE":
			deleteCalled = true
			w.WriteHeader(http.StatusNoContent)
		default:
			http.NotFound(w, r)
	...
	if !deleteCalled {
		t.Fatalf("expected DELETE /v3/simulations/nodes/interfaces/services/svc-1/ to be called")
	}
}

...
func TestFilterGPUNodes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: []string{"node-gpu-1", "node-gpu-2"},
		},
		{
			name: "no gpu nodes",
			input: []api.Node{
		...
		{
			name: "similar prefix but not gpu",
			input: []api.Node{
				{Name: "node-gpu-like-1"},
				{Name: "node-gpux-1"},
				{Name: "node-gpu-1"},
			},
			expected: []string{"node-gpu-like-1", "node-gpux-1", "node-gpu-1"},
		},
		{
			name: "case sensitive",
			input: []api.Node{
			...
			expected: []string{"node-gpu-1"},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := FilterGPUNodes(tt.input)

			if len(got) != len(tt.expected) {
				t.Errorf("FilterGPUNodes() length: got %d, want %d", len(got), len(tt.expected))
				return
			}

			for i, node := range got {
				if node.Name != tt.expected[i] {
					t.Errorf("FilterGPUNodes() at index %d: got %s, want %s", i, node.Name, tt.expected[i])
				}
			}
		})
	}
...
func BenchmarkFilterGPUNodes(b *testing.B) {
	nodes := []api.Node{
		{Name: "node-gpu-1"},
		{Name: "node-storage-1"},
		{Name: "node-gpu-2"},
